...
CREATE DATABASE IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_RAW_DB
  COMMENT = 'Raw data for ${PROJECT_CODE} (${ENV_CODE})';

CREATE DATABASE IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_DW_DB
  COMMENT = 'Data warehouse for ${PROJECT_CODE} (${ENV_CODE})';

-- ---------------------------------------------------------------------------
-- Schemas
-- ---------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_RAW_DB.SEED
  COMMENT = 'Seed data loaded by dbt';

CREATE SCHEMA IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_DW_DB.OBT
  COMMENT = 'One Big Table layer - pre-aggregated views';

CREATE SCHEMA IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_DW_DB.SEMANTIC
  COMMENT = 'Semantic views for Cortex Analyst';

-- ---------------------------------------------------------------------------
-- Warehouses
-- ---------------------------------------------------------------------------
CREATE WAREHOUSE IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_DBT_WH_XS
...
CREATE WAREHOUSE IF NOT EXISTS ${ENV_CODE}_${PROJECT_CODE}_CORTEX_WH_XS
  WAREHOUSE_SIZE = 'XSMALL'
  AUTO_SUSPEND   = 60
  AUTO_RESUME    = TRUE
  INITIALLY_SUSPENDED = TRUE
