resource "aws_api_gateway_rest_api" "lambda_proxy" {
  name        = "${local.resource_name_prefix}-${lower(var.snowflake_ext_function_name)}-api"
...
resource "aws_api_gateway_rest_api_policy" "lambda_proxy" {
  ...
  depends_on  = [aws_api_gateway_deployment.lambda_proxy, aws_iam_role.snow_ext_function, aws_api_gateway_rest_api.lambda_proxy]
}